...
def score_earnings_explosiveness(df):
    ...
    base = 0.45*e2 + 0.35*e1 + 0.20*e3

    # e1 = np.clip(df["earnings_explosiveness_z"] / 3, 0, 1)
    # e2 = np.clip(df["earnings_tail_z"] / 3, 0, 1)
    # # 0.15 = ~15% move is already violent (tune later)
    # e3 = np.clip(df["abs_reaction_p75"] / 0.15, 0, 1)
    # entropy already assumed 0–1
...
def score_momentum_fragility(df):
    ...
    m2 = np.clip(np.abs(df["directional_bias"].fillna(0)) / 0.05, 0, 1)
    m3 = np.clip(np.abs(df["sector_drift_60d"].fillna(0)) / 0.10, 0, 1)

    base = (
        0.45 * m1 +   # positioning pressure
        0.35 * m2 +   # directional skew
        0.20 * m3    # sector trend maturity
    )
